...
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        # FIX: resolve_peer BOT'LARDA YASAK → 400 BOT_METHOD_INVALID
        # Güvenli OWNER belirleme
        try:
            _owner = (
                OWNER_ID[0]
                if isinstance(OWNER_ID, (list, tuple)) and len(OWNER_ID) > 0
                else (OWNER_ID if isinstance(OWNER_ID, int) else None)
            )
        except Exception:
            _owner = None

        # İsteğe bağlı: get_users ile doğrula (başarısız olursa ID'yi direkt kullan)
        if _owner is not None:
            try:
                owner_user = await app.get_users(_owner)
                OWNER = owner_user.id
            except Exception:
                OWNER = _owner
        else:
